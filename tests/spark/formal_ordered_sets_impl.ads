...
generic
   ...
   with function "=" (E1, E2 : Element_Type) return Boolean is <>;
package Formal_Ordered_Sets_Impl with SPARK_Mode is
   ...
   function Capacity (Self : Base_Set'Class) return Count_Type
      with Import;
   ...
   function Element (Self : Base_Set'Class; Position : Cursor) return
     Element_Type
   with
     Import,
   ...
   with
     ...
     (Contains (Self, Element) => Has_Element (Self, Find'Result)
      and Formal_Ordered_Sets_Impl.Element (Self, Find'Result) = Element,
   ...
   --  Insert an element Element in Self if Element is not already in present.
     ...
     (Contains (Self, Element) =>
      ...
      and (for all I in 1 .. Length (Self) - 1 =>
          (if I < P_Get (Positions (Self), Find (Self, Element))
      ...
      and (for all Position of Positions (Self)'Old =>
        ...
        and Formal_Ordered_Sets_Impl.Element (Self, Position) =
      ...
      and (for all Position of Positions (Self) =>
        ...
        or Formal_Ordered_Sets_Impl.Element (Self, Position) = Element)

      --  Cursors designating elements smaller than Element in Self are
      --  preserved.

      and (for all Position of Positions (Self)'Old =>
          (if Formal_Ordered_Sets_Impl.Element (Self, Position) < Element
           then P_Get (Positions (Self), Position) =
             P_Get (Positions (Self)'Old, Position)

           --  Other cursors are shifted by 1.

           else P_Get (Positions (Self), Position) =
               P_Get (Positions (Self)'Old, Position) + 1)));
   ...
   --  Remove an element Element of Self if it is present.
     ...
     (Contains (Self, Element) =>
      ...
      and (for all I in 1 .. Length (Self) =>
          (if I < P_Get (Positions (Self)'Old, Find (Self, Element)'Old)
      ...
      and (for all Position of Positions (Self)'Old =>
        ...
        or E.Get (Elements (Self)'Old,
            P_Get (Positions (Self)'Old, Position)) = Element)
      ...
      and (for all Position of Positions (Self) =>
          (if Formal_Ordered_Sets_Impl.Element (Self, Position) < Element
           then P_Get (Positions (Self)'Old, Position) =
               P_Get (Positions (Self), Position)

           --  Other cursors are shifted by 1.

           else P_Get (Positions (Self)'Old, Position) =
               P_Get (Positions (Self), Position) + 1)),
   ...
   function First_Primitive (Self : Base_Set) return Cursor
   with Import;
